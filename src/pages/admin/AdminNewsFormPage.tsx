...
import { createNews, updateNews, fetchNewsDetail, deleteNewsFile, type NewsFile } from '@/api/news'
...
export default function AdminNewsFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await updateNews(Number(id), title, content, newFiles.length > 0 ? newFiles : undefined)
        alert('수정되었습니다.')
      } else {
        await createNews(title, content, newFiles.length > 0 ? newFiles : undefined)
        alert('등록되었습니다.')
      }
      navigate('/admin/news')
    } catch (err: unknown) {
  ...
  return (
    <div className="adm_wrap">
      <AdminSidebar />
      <div className="adm_content">
        <AdminHeader pageTitle={isEdit ? '소식 수정' : '소식 등록'} />
